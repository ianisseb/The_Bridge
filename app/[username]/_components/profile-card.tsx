...
export const ProfileCard = ({ seller, reviews }: ProfileCardProps) => {
  const languages = useQuery(api.users.getLanguagesByUsername, {
    username: seller.username,
  });
  const country = useQuery(api.users.getCountryByUsername, {
    username: seller.username,
  });
    ...
    <div className="flex space-x-4">
      ...
      <div className="w-[300px] flex flex-col justify-between gap-y-1">
        ...
        <div className="flex items-center gap-x-1">
          ...
          <div className="bg-yellow-100 text-red-900 font-semibold p-1 ml-3">
            {seller.customTag}
          </div>
        </div>
        <p>{seller.title}</p>
