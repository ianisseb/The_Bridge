...
export const ReviewsStats = ({ reviews }: ReviewsStatsProps) => {
  ...
  const ratingBreakdown = {
    "Seller communication level": calculateAverage(
      reviews.map((review) => review.communication_level)
    ),
    "Recommend to a friend": calculateAverage(
      reviews.map((review) => review.recommend_to_a_friend)
    ),
    "Service as described": calculateAverage(
      reviews.map((review) => review.service_as_described)
    ),
  };
    ...
    <div className="flex space-x-12">
      <div className="w-full md:w-1/2">
        <p className="font-bold">
          Average Review Score: {averageScore.toFixed(2)} stars
        </p>
      ...
      <div className="w-full md:w-1/2">
        ...
        {Object.entries(ratingBreakdown).map(([label, value], index) => (
          <p
            key={index}
            className="flex items-center text-gray-600 font-semibold mt-2"
          >
